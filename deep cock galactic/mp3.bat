...
for /r "C:\Program Files (x86)\Steam\steamapps\common\Deep Rock Galactic\FSD\Content\Paks\Nowy folder2\FSD\Content\Audio\" %%a in (.) do (FOR %%e IN ("%%~da%%~pa%%~na\*.ogg") DO (echo mp3 ^(!CURRENT!\%COUNT%^) & "ffmpeg.exe" -hide_banner -loglevel error -i "%%e" -acodec libmp3lame -q:a 0 -y "%%~de%%~pe%%~ne.mp3" & SET /A CURRENT+=1))
